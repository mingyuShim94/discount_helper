...
export function calculateOptimalDiscounts(
  ...
  if (
    filter.useKakaoPay &&
    discountRules.kakaoPayDiscount &&
    discountRules.kakaoPayDiscount.enabled
  ) {
    const kakaoPayDiscount = discountRules.kakaoPayDiscount;
    const discountRate = kakaoPayDiscount.discountRate;
    const discountAmount = amount * discountRate;

    // 최대 할인 금액 제한 적용
    let finalDiscountAmount = discountAmount;
    if (
      kakaoPayDiscount.maxDiscountAmount &&
      discountAmount > kakaoPayDiscount.maxDiscountAmount
    ) {
      finalDiscountAmount = kakaoPayDiscount.maxDiscountAmount;
    }

    const restrictions = kakaoPayDiscount.restrictions
      ? kakaoPayDiscount.restrictions.join(", ")
      : "카카오페이 결제 시 적용";

    results.push({
      method: "카카오페이 할인",
      description: `${discountRate * 100}% 즉시 할인`,
      originalAmount: amount,
      instantDiscountAmount: finalDiscountAmount,
      futureDiscountAmount: 0,
      totalBenefitAmount: finalDiscountAmount,
      finalAmount: amount - finalDiscountAmount,
      perceivedAmount: amount - finalDiscountAmount,
      discountRate: finalDiscountAmount / amount,
      rank: 1,
      note: restrictions,
    });
  }
  ...
  if (
    ...
    if (
      ...
      const kakaoPayDiscount = discountRules.kakaoPayDiscount;
      const discountRate = kakaoPayDiscount.discountRate;
      const discountAmount = remainingAmount * discountRate;

      // 최대 할인 금액 제한 적용
      let kakaoPayDiscountAmount = discountAmount;
      if (
        kakaoPayDiscount.maxDiscountAmount &&
        discountAmount > kakaoPayDiscount.maxDiscountAmount
      ) {
        kakaoPayDiscountAmount = kakaoPayDiscount.maxDiscountAmount;
      }
      ...
      const carrierRestrictions = typedCarrierInfo.restrictions
        ? typedCarrierInfo.restrictions.join(", ")
        : "1일 1회 사용 가능, 행사상품/담배/주류 제외";

      const kakaoPayRestrictions = kakaoPayDiscount.restrictions
        ? kakaoPayDiscount.restrictions.join(", ")
        : "카카오페이 결제 시 적용";
      ...
      results.push({
        ...
        description: `1,000원당 ${
          typedCarrierInfo.discountRate * 1000
        }원 할인 후 카카오페이로 ${discountRate * 100}% 추가 할인`,
        originalAmount: amount,
        instantDiscountAmount: carrierDiscountAmount + kakaoPayDiscountAmount,
        futureDiscountAmount: 0,
        ...
        note: `${carrierRestrictions}. ${kakaoPayRestrictions}`,
      });
    }
  }
